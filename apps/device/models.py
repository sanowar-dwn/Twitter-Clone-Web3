...
class Device(models.Model):
    ...
    device_log = models.TextField(blank=True)
...
# Updating the device log field everytime a device is returned
@receiver(post_save, sender=DeviceAssignment)
def update_device_log(sender, instance, **kwargs):
    device = instance.device
    if instance.return_date:
        device.device_log += (
        'condition at handout: ' + str(instance.condition_at_checkout) + ', '
        'condition at return: ' + str(instance.condition_at_return)
        )
        device.save()
    else:
        pass
